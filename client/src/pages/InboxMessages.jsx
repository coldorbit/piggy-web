...
export function VirtualizedMessageList({
  ...
  const selectedIndex = selectedMessage
    ...
    : -1;
  const selectedMessageId = selectedMessage ? String(selectedMessage.id) : null;

  useEffect(() => {
    if (selectedIndex < 0 || !selectedMessageId) {
      lastScrolledMessageIdRef.current = null;
      return;
    }
    if (lastScrolledMessageIdRef.current === selectedMessageId) return;

    lastScrolledMessageIdRef.current = selectedMessageId;
