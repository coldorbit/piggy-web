...
export function userAttributesFromBody(body, { requirePassword }) {
  ...
  if (!['admin', 'user', 'internal', 'bidder', 'readonly_bidder', 'editable_bidder', 'caller'].includes(role)) {
    throw new InputError('Role must be admin, user, internal, caller, readonly_bidder, or editable_bidder');
  }
